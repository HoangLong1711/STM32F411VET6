...
int main(void)
{
  ...
  {
		for (int i = 0; i <= 7; i++) {
			pushHEX(colsData[3][i]);
			pushHEX(colsData[2][i]);
			pushHEX(colsData[1][i]);
			pushHEX(rowsSelect[i]);
			LATCH();
			HAL_Delay(1);
		}
//	  			pushHEX(0xC3);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_11|GPIO_PIN_13|GPIO_PIN_15, GPIO_PIN_RESET);

  /*Configure GPIO pins : PD11 PD13 PD15 */
